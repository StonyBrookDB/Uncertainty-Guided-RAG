...
for i in range(1, 6):
    df = pd.read_csv(f"results-and-visualizations/rag_iterative_v2/1_complete/medmcqa_{i}.csv")
    df['source'] = 'MEDMCQA'
    df['split'] = i
    iterated_df = pd.concat([iterated_df, df], ignore_index=True)

for i in range(1, 6):
    df = pd.read_csv(f"results-and-visualizations/rag_iterative_v2/1_complete/mmlu_{i}.csv")
    df['source'] = 'MMLU'
    df['split'] = i
    iterated_df = pd.concat([iterated_df, df], ignore_index=True)
